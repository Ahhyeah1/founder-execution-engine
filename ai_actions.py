...
def offline_actions(goal_text: str, difficulty: int) -> List[Dict[str, Any]]:
    goal = goal_text.strip()
    lower = goal.lower()
    actions: List[Dict[str, Any]] = []

    def add(text: str, w: float, d: int):
        actions.append({"text": text, "impact_weight": float(w), "difficulty": int(d), "non_negotiable": True})

    if any(k in lower for k in ["mrr", "sales", "customers", "customer", "revenue", "sell", "pipeline"]):
        add("Contact 10 prospects with ONE offer. Log replies.", 1.4, min(3, difficulty + 1))
        ...
        add("Ask for money: send 1 checkout link/invoice or request a deposit.", 1.5, 3)
        ...
        add("Post a public update showing what you shipped.", 1.1, difficulty)
    ...
    actions = actions[:5]
    if len(actions) < 3:
        actions.append({"text": "Ship a result you can show publicly.", "impact_weight": 1.2, "difficulty": difficulty, "non_negotiable": True})
    return actions
...
- At least 1 action must be uncomfortable (contacting people, publishing, asking for money, committing publicly).
